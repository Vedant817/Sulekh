...
- [x] **3.1 Generation Orchestrator.** Per-section pipeline: assemble (confirmed data + retrieved requirements + template) → Claude draft → provenance + coverage map. Dependency-ordered.
  - **Accept:** sections generate in correct dependency order (e.g. Capital Structure before Basis for Issue Price); each produces prose + persisted provenance. ✓ Verified against real DB (`orchestrator.ts`, injected drafter): generation runs in dependency-respecting topological order (capital-structure→objects-of-issue→basis-for-issue-price; financial-information→mda; business-overview→risk-factors), each section persists a draft + a `section_provenance` row referencing the intake fields used (integration test asserts provenance count == section count and includes `legal_name`). Live Claude prose exercised once a key exists (drafter is `claude-drafter.ts`).
- [ ] **3.2 Grounding & gap-marking.** Model constrained to supplied facts; emits `[[GAP: …]]` for unknowns instead of inventing.
  - **Accept:** with a deliberately-omitted fact, the section shows a GAP marker for it and does **not** fabricate a value (verified by inspection). _(Built: strict grounding system prompt (use only supplied data; GAP-mark unknowns), `[[GAP: …]]` parser + `stripGapMarkers` (so a topic mentioned only in a gap doesn't count as covered) — unit-verified; gap markers persist as `gap_flags` (integration-verified). Confirming the live model actually GAP-marks rather than fabricates needs a real `ANTHROPIC_API_KEY`. Box stays unchecked until observed on live output.)_
...
- [x] **3.5 Model routing.** Narrative → `MODEL_DRAFTING`; reasoning-heavy sections → `MODEL_REASONING`; both from env.
  - **Accept:** logs show the correct model per section; changing env swaps models without code change. ✓ Verified: `modelKindForSection` routes risk-factors / basis-for-issue-price / mda / financial-information → reasoning, others → drafting (unit tests); the orchestrator records `model_used` per section and `claude-drafter.ts` resolves the kind to `getModels().{drafting,reasoning}` (from `MODEL_DRAFTING`/`MODEL_REASONING` env) — swapping env swaps models with no code change.
- [ ] **3.6 GATE — Phase 3 exit.** A full draft (all §4.3 sections) generates for the sample issuer from real intake — every section grounded, traceable, gaps marked not invented.
  - **Accept:** open the generated draft; each section links to its provenance; no hallucinated facts on a spot audit of 5 sections. _(Orchestration, provenance, coverage, gap-flagging, and progress all verified against the real DB with an injected drafter; the full grounded draft from real Claude for the seeded sample issuer needs `ANTHROPIC_API_KEY` (+ the sample issuer seed, task 7.4). GATE deferred until live generation is run.)_
